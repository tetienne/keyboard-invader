  ...
  enter(ctx: GameContext): void {
    void (async () => {
      try {
        ...
        // Show user-visible error (use canvas Text since BitmapFont may not be loaded)
...
export class PlayingState implements GameState {
    ...
    for (let i = this.activeEntities.length - 1; i >= 0; i--) {
      ...
      if (entity?.markedForRemoval && entity.tween === null) {
        ...
        if (this.entitiesContainer) {
          this.entitiesContainer.removeChild(entity.container)
        } else {
  ...
  private _spawnLetter(ctx: GameContext): void {
    ...
    ac.setLetter(letter.toUpperCase(), tint)
    ac.setTexture(AlienContainer.getRandomAlienTexture(false))
    ac.scale.set(1)
    ac.alpha = 1
    ac.x = 80 + Math.random() * (BASE_WIDTH - 160)
    ac.y = -40
    ac.visible = true
    ...
    for (const key of keys) {
      ...
      if (match) {
        ...
        if (this.defender && this.laser) {
          ...
          this.laser.fire(defPos.x, defPos.y, match.container.x, match.container.y)
        }
        // Destruction burst at alien position
        this.effects?.burst(match.container.x, match.container.y, match.originalTint)
  ...
  private _processWordInput(keys: string[]): void {
    ...
    for (const key of keys) {
      ...
      if (result === 'correct') {
        ...
        if (this.defender && this.laser) {
          const defPos = this.defender.getPosition()
          this.laser.fire(defPos.x, defPos.y, activeWord.container.x, activeWord.container.y)
        }
        // Destruction burst
        this.effects?.burst(activeWord.container.x, activeWord.container.y, activeWord.originalTint)
